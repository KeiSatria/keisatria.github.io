  ...
  <br><br>
    ...
    <div class="card-title text-center">
      <a href="pdftabungan.php" target="_blank" class="btn btn-success"><i class="fa fa-file-pdf-o"></i> &nbsp PRINT DATA INI</a>
  </div>
      
        <input type="text" class="form-control form-control-sm" placeholder="Cari Berdasarkan Nomor Tabungan..." id="getSearch" onkeyup="getFunction()">
        
      <!--Table Data Siswa-->
      <table id="getTable" class="table table-bg-3 table-stripped bg-3">
            <thead>
              <tr>
                <th scope="col">No Tabungan</th>
        ...
        <?php 
          $data = mysqli_query($config,"select * from data_tabungan order by nama asc");
          while($tampilkan = mysqli_fetch_array($data)){
        ?>
        
              <tr>
                <td><?php echo $tampilkan["no_tabungan"] ;?></td>
                <td><?php echo $tampilkan["nama"] ;?></td>
                <td><?php echo $tampilkan["saldo"] ;?></td>
                
                <td>
                  
              
          <a href="edit2.php?id=<?php echo $tampilkan["no_tabungan"] ;?>" class="custom-button btn-sm text-white">Ubah</a>
          <a href="admin2.php?delete=<?php echo $tampilkan["no_tabungan"] ;?>"onclick="return confirm('Apakah Anda Ingin Menghapus Data Ini?');" class="custom-button btn-sm text-white">Hapus</a>
